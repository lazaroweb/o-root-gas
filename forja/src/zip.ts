// Gerador de ZIP mínimo (método "store", sem compressão), 100% no cliente e sem
// dependências externas. Suficiente pra empacotar arquivos de texto (SKILL.md,
// README) num .zip válido que o Finder/Explorer/IDEs abrem normalmente.

export interface ZipEntry {
  path: string;
  content: string;
}

// CRC-32 (polinômio padrão do ZIP). Tabela pré-computada pra não recalcular.
const CRC_TABLE: number[] = (() => {
  const table: number[] = [];
  for (let n = 0; n < 256; n++) {
    let c = n;
    for (let k = 0; k < 8; k++) c = c & 1 ? 0xedb88320 ^ (c >>> 1) : c >>> 1;
    table[n] = c >>> 0;
  }
  return table;
})();

function crc32(bytes: Uint8Array): number {
  let crc = 0xffffffff;
  for (let i = 0; i < bytes.length; i++) {
    crc = CRC_TABLE[(crc ^ bytes[i]) & 0xff] ^ (crc >>> 8);
  }
  return (crc ^ 0xffffffff) >>> 0;
}

// Monta um Blob .zip a partir de uma lista de arquivos de texto.
export function criarZipBlob(entries: ZipEntry[]): Blob {
  const enc = new TextEncoder();
  const partes: Uint8Array[] = [];
  const central: Uint8Array[] = [];
  let offset = 0;

  for (const e of entries) {
    const nome = enc.encode(e.path);
    const dados = enc.encode(e.content);
    const crc = crc32(dados);
    const tam = dados.length;

    // Local file header (30 bytes + nome)
    const lh = new Uint8Array(30 + nome.length);
    const dv = new DataView(lh.buffer);
    dv.setUint32(0, 0x04034b50, true);
    dv.setUint16(4, 20, true); // versão mínima
    dv.setUint16(6, 0x0800, true); // flag: nomes em UTF-8
    dv.setUint16(8, 0, true); // método: store
    dv.setUint16(10, 0, true); // hora
    dv.setUint16(12, 0, true); // data
    dv.setUint32(14, crc, true);
    dv.setUint32(18, tam, true); // comprimido
    dv.setUint32(22, tam, true); // descomprimido
    dv.setUint16(26, nome.length, true);
    dv.setUint16(28, 0, true); // extra
    lh.set(nome, 30);
    partes.push(lh, dados);

    // Central directory header (46 bytes + nome)
    const cd = new Uint8Array(46 + nome.length);
    const cv = new DataView(cd.buffer);
    cv.setUint32(0, 0x02014b50, true);
    cv.setUint16(4, 20, true); // versão criadora
    cv.setUint16(6, 20, true); // versão mínima
    cv.setUint16(8, 0x0800, true);
    cv.setUint16(10, 0, true);
    cv.setUint16(12, 0, true);
    cv.setUint16(14, 0, true);
    cv.setUint32(16, crc, true);
    cv.setUint32(20, tam, true);
    cv.setUint32(24, tam, true);
    cv.setUint16(28, nome.length, true);
    cv.setUint16(30, 0, true); // extra
    cv.setUint16(32, 0, true); // comentário
    cv.setUint16(34, 0, true); // disco
    cv.setUint16(36, 0, true); // attrs internos
    cv.setUint32(38, 0, true); // attrs externos
    cv.setUint32(42, offset, true); // offset do local header
    cd.set(nome, 46);
    central.push(cd);

    offset += lh.length + dados.length;
  }

  const tamCentral = central.reduce((a, c) => a + c.length, 0);
  const offsetCentral = offset;

  // End of central directory
  const eocd = new Uint8Array(22);
  const ev = new DataView(eocd.buffer);
  ev.setUint32(0, 0x06054b50, true);
  ev.setUint16(4, 0, true);
  ev.setUint16(6, 0, true);
  ev.setUint16(8, entries.length, true);
  ev.setUint16(10, entries.length, true);
  ev.setUint32(12, tamCentral, true);
  ev.setUint32(16, offsetCentral, true);
  ev.setUint16(20, 0, true);

  return new Blob([...partes, ...central, eocd] as unknown as BlobPart[], { type: 'application/zip' });
}

// Dispara o download de um Blob com o nome dado.
export function baixarBlob(blob: Blob, nomeArquivo: string): void {
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = nomeArquivo;
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
}
