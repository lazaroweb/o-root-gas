// ApisPanel — gestão de endpoints/APIs (CRUD + status ao vivo).
//
// Centralização de conexões (v1.188.0): este painel é a fonte ÚNICA de cadastro
// de endpoints. Vive em Configurações → Conexões → "APIs & Webhooks" no modo
// 'full' (cria/edita/remove) e em Operações → Status no modo 'monitor' (só lê,
// testa e leva pro hub). Antes esse CRUD ficava embutido no OpsStatus.
import React, { useState, useEffect } from 'react';
import { Button, Modal, Form, Input, Select, Tag, Empty, Spin, Popconfirm, App as AntApp } from 'antd';
import { Plus, Trash2, Pencil, Zap, ExternalLink, Server, Settings as SettingsIcon, RefreshCw } from 'lucide-react';
...
interface ApisPanelProps {
  ...
  // 'monitor' = só leitura + teste + atalho pro hub (Operações).
  mode?: 'full' | 'monitor';
  // Em modo monitor, leva o usuário pro hub de cadastro.
  onGerenciar?: () => void;
}

export default function ApisPanel({ mode = 'full', onGerenciar }: ApisPanelProps): React.ReactElement {
  const t = useTokens();
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginBottom: 14 }}>
        <Button icon={<RefreshCw size={15} />} loading={validating} onClick={validar}>Revalidar</Button>
        {isFull ? (
          <Button type="primary" icon={<Plus size={16} />} onClick={() => handleOpen()}>Cadastrar API</Button>
        ) : onGerenciar ? (
          <Button type="primary" icon={<SettingsIcon size={15} />} onClick={onGerenciar}>Gerenciar em Configurações</Button>
        ) : null}
      </div>
      ...
      <Panel title={<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><Server size={18} strokeWidth={1.6} color={t.accents.peach} /> Endpoints por aplicação</span>} padding={8}>
        {loading ? (
          ...
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={isFull ? 'Nenhuma API cadastrada' : 'Nenhuma API cadastrada — cadastre em Configurações → Conexões.'} style={{ padding: 32 }}>
            {isFull && <Button type="primary" icon={<Plus size={16} />} onClick={() => handleOpen()}>Cadastrar API</Button>}
            {!isFull && onGerenciar && <Button type="primary" icon={<SettingsIcon size={15} />} onClick={onGerenciar}>Ir para Configurações</Button>}
          </Empty>
        ) : (
          grupos.map(grupo => (
                ...
                return (
                  <div key={api.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 14px', borderBottom: `1px solid ${t.borderSoft}` }}>
                    ...
                    <div style={{ display: 'flex', gap: 2 }}>
                      ...
                      {isFull && <Button type="text" size="small" icon={<Pencil size={15} />} onClick={() => handleOpen(api)} />}
                      {isFull && (
      ...
      <div style={{ marginTop: 16, color: t.textTertiary, fontSize: 12.5, paddingLeft: 4 }}>
        ...
        {!isFull && ' O cadastro vive em Configurações → Conexões → APIs & Webhooks.'}
      </div>
