...
export default function FinPessoal(): React.ReactElement {
  ...
  const NAV: Array<{
    ...
    { key: 'familia', icon: Users, label: 'Família', count: membros.length, accent: 'lavender', group: 'Organização', desc: 'Quem divide contas no seu cartão — quem deve e quem pagou.' },
...
// ─── Sub-view: visão mensal ──────────────────────────────────────────────────

// Donut de gastos (SVG, sem libs de chart). Cada fatia é hoverável: ao passar o
// mouse, a fatia destaca, mostra tooltip nativo (<title>) e o centro passa a
// exibir aquela categoria (label, valor e %). Fonte central enxuta pra respirar.
function DonutGastos({ segmentos, total, label }: {
  segmentos: Array<{ cor: string; valor: number; label: string }>; total: number; label: string;
}): React.ReactElement {
  const t = useTokens();
  ...
  let acc = 0;
  const segs = segmentos.filter((s) => s.valor > 0).map((s) => {
  ...
  const centroLabel = hov ? hov.label : label;
  const centroValor = hov ? hov.valor : total;
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)' }}>
        ...
        {segs.map((s, i) => {
          ...
          return (
            ...
            >
              <title>{`${s.label}: ${formatBRL(s.valor)} (${(s.frac * 100).toFixed(0)}%)`}</title>
            </circle>
          );
        })}
      </svg>
      <div style={{
        ...
        <span style={{ fontFamily: FONTS.display, fontSize: 14.5, fontWeight: 600, color: t.text, fontVariantNumeric: 'tabular-nums', marginTop: 2 }}>{formatBRL(centroValor)}</span>
        {centroPct !== null && (
          <span style={{ fontFamily: FONTS.ui, fontSize: 10.5, color: t.textTertiary, marginTop: 1 }}>{centroPct.toFixed(0)}% do mês</span>
        )}
      </div>
    </div>
  );
...
function VisaoMensal({ resumo, loading, orcamentos, lancamentos, cartoes, mes, onRecarregar, onEditar, onIrParaOrcamentos }: {
  ...
  const labelCategoria = useLabelCategoria();
  const categorias = useContext(CategoriasContext);
  // Drawer de composição de uma categoria: lista os lançamentos do mês daquela
  // categoria (com cartão/data/valor) e permite recategorizar inline.
  ...
  const recategorizar = async (l: LancamentoPessoal, novaCat: string) => {
    ...
    if (res?.ok) {
      // Aprende a regra (comércio → categoria) e reaplica nos itens iguais do mês.
  ...
  const reclassificarIA = async () => {
    ...
    if (res?.ok) {
      const d = (res.data as { porRegra: number; porIA: number; restantes: number } | undefined) || { porRegra: 0, porIA: 0, restantes: 0 };
      const total = d.porRegra + d.porIA;
      onRecarregar();
      modal.success({
        title: 'Reclassificação concluída',
        content: (
          <div style={{ fontFamily: FONTS.ui, fontSize: 13.5, lineHeight: 1.7 }}>
            <div><strong>{total}</strong> lançamento{total === 1 ? '' : 's'} reclassificado{total === 1 ? '' : 's'}:</div>
            <div style={{ marginTop: 6 }}>• <strong>{d.porRegra}</strong> por regras já aprendidas</div>
            <div>• <strong>{d.porIA}</strong> pela IA (e viraram regra pro futuro)</div>
            <div style={{ marginTop: 6, color: t.textTertiary }}>
              {d.restantes > 0
                ? `${d.restantes} continuam em "Outros" — recategorize manualmente que eu aprendo.`
                : 'Nenhum item ficou em "Outros". 🎉'}
            </div>
          </div>
        ),
        okText: 'Beleza',
  ...
  const catInfoDetalhe = catDetalhe ? resolverCategoria(catDetalhe, categorias) : null;

  if (loading && !resumo) {
    return (
      <Panel padding={32}>
        <div style={{ textAlign: 'center', color: t.textTertiary }}>Carregando…</div>
      </Panel>
    );
  }
  if (!resumo || resumo.totalLancamentos === 0) {
    return (
      <Panel padding={32}>
        <Empty
          description={
            <div>
              ...
              <div style={{ color: t.textTertiary, fontFamily: FONTS.ui, fontSize: 12.5, marginTop: 4 }}>
                Clique em <strong>"Novo lançamento"</strong> no topo pra começar.
              </div>
            </div>
          }
        />
  ...
  // Ordena categorias por valor decrescente e resolve ícone outline + cor.
  const cats = Object.entries(resumo.porCategoria).sort((a, b) => b[1] - a[1]);
  const catsResolvidas = cats.map(([nome, valor]) => {
    const c = resolverCategoria(nome, categorias);
    return { nome, valor, cor: c.cor, icone: c.icone, label: c.label || nome };
  });
  const metodos = Object.entries(resumo.porMetodo).sort((a, b) => b[1] - a[1]);
  // Nº de lançamentos por método (despesas do mês) — deixa o widget "Como você
  // pagou" auto-explicativo: além do valor/%, mostra quantas compras em cada meio.
  const metodoCount = lancamentos.reduce((acc, l) => {
    if (l.tipo !== 'despesa') return acc;
    const m = l.metodo || 'outro';
    acc[m] = (acc[m] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  // Orçamentos ativos pra exibir como alerta no topo (estouro/atenção)
  const orcamentosCriticos = orcamentos?.itens.filter((o) => o.status !== 'ok') || [];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {orcamentosCriticos.length > 0 && (
        <Alert
          ...
          icon={<Target size={16} />}
          message={
            <div style={{ fontFamily: FONTS.ui, fontSize: 13 }}>
              <strong>{orcamentosCriticos.length}</strong> orçamento(s) {orcamentosCriticos.some((o) => o.status === 'estouro') ? 'estouraram' : 'em atenção'} esse mês:{' '}
              {orcamentosCriticos.slice(0, 3).map((o) => labelCategoria(o.categoria)).join(', ')}
      ...
      {/* Hero: donut de categorias + ranking premium (ícones outline, sem emoji).
          Altura limitada (≈ a do menu Panorama) com a lista de categorias rolando
          internamente — a página não estica sem fim com muitas categorias. */}
      <div style={{
        background: `linear-gradient(135deg, ${t.accents.peach}1a, ${t.surface} 60%)`,
        border: `1px solid ${t.borderSoft}`, borderRadius: 16, padding: 22,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, flex: '0 0 auto' }}>
          <DonutGastos
            segmentos={catsResolvidas.map((c) => ({ cor: c.cor, valor: c.valor, label: c.label }))}
            total={resumo.totalDespesas}
            label="Gasto do mês"
          />
          <div style={{ fontFamily: FONTS.ui, fontSize: 11.5, color: t.textTertiary }}>
            {cats.length} categoria{cats.length === 1 ? '' : 's'} · {resumo.totalLancamentos} lançamento{resumo.totalLancamentos === 1 ? '' : 's'}
          </div>
        </div>
        <div style={{ flex: '1 1 340px', minWidth: 280, display: 'flex', flexDirection: 'column', gap: 12, alignSelf: 'stretch' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
            <div style={{ fontFamily: FONTS.display, fontSize: 17, fontWeight: 600, color: t.text }}>Por categoria</div>
            {catsResolvidas.some((c) => c.nome === 'outros') && (
              <Button
                size="small" icon={<Sparkles size={13} />} loading={reclassificando}
                onClick={() => void reclassificarIA()}
                style={{ color: t.accents.lavender, borderColor: `${t.accents.lavender}66` }}
              >
                {reclassificando ? 'Reclassificando…' : 'Reclassificar "Outros" com IA'}
              </Button>
            )}
          </div>
          <div className="forja-scroll-y" style={{ display: 'flex', flexDirection: 'column', gap: 14, maxHeight: 'min(56vh, 440px)', overflowY: 'auto', paddingRight: 6 }}>
          {catsResolvidas.length === 0 ? (
            <div style={{ fontFamily: FONTS.ui, fontSize: 13, color: t.textTertiary }}>Sem despesas neste mês.</div>
          ) : catsResolvidas.map((c) => {
            const pct = resumo.totalDespesas > 0 ? (c.valor / resumo.totalDespesas) * 100 : 0;
            return (
              <div
                key={c.nome}
                onClick={() => setCatDetalhe(c.nome)}
                title="Ver lançamentos desta categoria"
                style={{
                  display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer',
                  borderRadius: 10, padding: 6, margin: -6, transition: 'background 0.15s',
                }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.background = t.surfaceMuted; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.background = 'transparent'; }}
              >
                <IconeCategoria nome={c.icone} cor={c.cor} size={16} boxSize={34} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 5, gap: 8 }}>
                    <span style={{ fontFamily: FONTS.ui, fontSize: 13, color: t.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.label}</span>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'baseline', flexShrink: 0 }}>
                      <span style={{ fontFamily: FONTS.display, fontSize: 13.5, color: t.text, fontVariantNumeric: 'tabular-nums' }}>{formatBRL(c.valor)}</span>
                      <span style={{ fontSize: 11, color: t.textTertiary, fontFamily: FONTS.ui, minWidth: 34, textAlign: 'right' }}>{pct.toFixed(0)}%</span>
                    </div>
                  </div>
                  <div style={{ height: 7, background: t.surfaceMuted, borderRadius: 5, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${pct}%`, background: c.cor, borderRadius: 5, transition: 'width 0.4s' }} />
                  </div>
                </div>
      ...
      <Panel
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Wallet size={15} color={t.accents.peach} />
            <span>Como você pagou</span>
          </div>
        }
        padding={18}
      >
        <div style={{ fontFamily: FONTS.ui, fontSize: 12, color: t.textTertiary, marginTop: -4, marginBottom: 14 }}>
          Divisão dos gastos do mês por forma de pagamento (cartão, Pix, dinheiro, boleto…) — quanto saiu por cada meio.
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {metodos.map(([met, valor]) => {
            const pct = resumo.totalDespesas > 0 ? (valor / resumo.totalDespesas) * 100 : 0;
            const metInfo = METODOS.find((m) => m.value === met);
            const cor = metInfo?.cor || t.accents.peach;
            const nLanc = metodoCount[met] || 0;
            return (
              <div key={met} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  width: 34, height: 34, borderRadius: 10, background: `${cor}12`, border: `1px solid ${cor}33`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: cor,
                }}>
                  {metInfo?.icon}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 5, gap: 8 }}>
                    <span style={{ fontFamily: FONTS.ui, fontSize: 13, color: t.text }}>
                      {labelMetodo(met)}
                      {nLanc > 0 && <span style={{ fontSize: 11, color: t.textTertiary }}> · {nLanc} lançamento{nLanc === 1 ? '' : 's'}</span>}
                    </span>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'baseline', flexShrink: 0 }}>
                      <span style={{ fontFamily: FONTS.display, fontSize: 13.5, color: t.text, fontVariantNumeric: 'tabular-nums' }}>{formatBRL(valor)}</span>
                      <span style={{ fontSize: 11, color: t.textTertiary, fontFamily: FONTS.ui, minWidth: 34, textAlign: 'right' }}>{pct.toFixed(0)}%</span>
                    </div>
                  </div>
                  <div style={{ height: 7, background: t.surfaceMuted, borderRadius: 5, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${pct}%`, background: cor, borderRadius: 5, transition: 'width 0.4s' }} />
                  </div>
                </div>
              </div>
            );
      ...
      {/* Mini visão de orçamentos quando configurados — mostra as 4 maiores barras */}
      {orcamentos && orcamentos.itens.length > 0 && (
        <Panel
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Target size={15} color={t.accents.peach} />
              <span>Orçamentos do mês</span>
              <Tag color="orange" style={{ marginInlineEnd: 0 }}>{orcamentos.itens.length}</Tag>
            </div>
          }
          padding={16}
        >
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12 }}>
            {orcamentos.itens.slice(0, 6).map((o) => (
              <MiniBarraOrcamento key={o.id} item={o} />
            ))}
          </div>
        </Panel>
      )}

      {/* Modal de composição da categoria — lançamentos do mês + recategorizar */}
      <Modal
        open={!!catDetalhe}
        ...
        title={
          catInfoDetalhe ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <IconeCategoria nome={catInfoDetalhe.icone} cor={catInfoDetalhe.cor} size={16} boxSize={32} />
              <div>
                <div style={{ fontFamily: FONTS.display, fontSize: 16, fontWeight: 600, color: t.text }}>{catInfoDetalhe.label}</div>
...
// ─── Mini barra de orçamento (usada na visão geral) ──────────────────────────

function MiniBarraOrcamento({ item }: { item: ProgressoOrcamentoItem }): React.ReactElement {
  const t = useTokens();
  const labelCategoria = useLabelCategoria();
  const cor =
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
        <span style={{ fontFamily: FONTS.ui, fontSize: 12.5, color: t.text }}>{labelCategoria(item.categoria)}</span>
        <span style={{ fontFamily: FONTS.display, fontSize: 12, color: cor, fontVariantNumeric: 'tabular-nums', fontWeight: 500 }}>
          {item.pct.toFixed(0)}%
        </span>
      </div>
      <div style={{ height: 6, background: t.surface, borderRadius: 4, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${Math.min(100, item.pct)}%`, background: cor, transition: 'width 0.3s' }} />
      </div>
      <div style={{ fontSize: 10.5, color: t.textTertiary, fontFamily: FONTS.ui, marginTop: 4 }}>
        {formatBRL(item.gasto)} de {formatBRL(item.limite)} · resta {formatBRL(item.restante)}
      </div>
    </div>
  );
}
