...
import { Button, Select, Switch, App as AntApp, Skeleton, Segmented, Tooltip, Input, Empty, Drawer, Upload, ColorPicker, InputNumber, Tag } from 'antd';
import {
...
export default function Relatorios(): React.ReactElement {
  ...
  const NAV: SubNavItem<typeof secao>[] = [
    ...
    { key: 'exportar', icon: Download, label: 'Exportar', accent: 'lavender', desc: 'Pontos de restauração do sistema, CSV por entidade e resumo financeiro por e-mail.' },
  ];
...
// Um ponto de restauração do sistema completo (cópia integral da planilha-banco
// no Drive) — registro devolvido por listarBackupsCompletos.
...
// (todas as tabelas, incluindo o Financeiro Pessoal) com restore de um clique.
// "Fiz merda? Volto exatamente pra aquele momento." Um snapshot 'pre-restore' é
// criado automaticamente antes de qualquer restauração.
function PontosRestauracaoPanel(): React.ReactElement {
  const t = useTokens();
...
function ExportarPanel(): React.ReactElement {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Panel padding={20} title={<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><FileJson size={17} color={t.accents.blue} /> Exportar JSON (QG do negócio)</span>}>
        <p style={{ color: t.textSecondary, fontSize: 13, lineHeight: 1.6, marginTop: 0 }}>
          Baixa um JSON com as entidades do QG (sistemas, clientes, ideias, custos, receitas, APIs,
          alertas, stacks, decisões, riscos, timeline, oportunidades) pra arquivar uma cópia local ou
          subir pro Git. <strong>Não inclui o Financeiro Pessoal</strong> — pra proteção completa com
          restauração, use os Pontos de restauração acima.
        </p>
